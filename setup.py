...
class BuildInstaller(Command):
    ...
    def run(self):
        ...
        postinstall_src = os.path.join(
            os.path.dirname(__file__), 'postinstall.sh'
        )
...
class BuildAppImage(Command):
    description = 'Build the executable and create an AppImage'
    user_options = []
    ...
    def run(self):
        ...
        # AppImage requires an AppDir with specific structure
        bin_dir = os.path.join(app_dir, 'usr', 'bin')
        os.makedirs(bin_dir, exist_ok=True)

        # Copy frozen build output
...
"""
        ...
        with open(
            os.path.join(
                app_dir, 'usr', 'share', 'applications', 'TimeMatters.desktop'
            ),
            'w',
        ) as f:
        ...
        icon_src = os.path.join(
            'src', 'assets', 'tmsa3.png'
        )  # AppImage prefers PNG
        icon_dest = os.path.join(app_dir, 'TimeMatters.png')
        shutil.copy2(icon_src, icon_dest)

        # Make AppImage
        appimage_name = f'TimeMatters-{VERSION}-x86_64.AppImage'
        subprocess.check_call(['appimagetool', app_dir, appimage_name])

        # Optional: Generate .zsync
        if shutil.which('zsyncmake'):
            subprocess.check_call(['zsyncmake', appimage_name])


cmdclass = {
    'build_installer': BuildInstaller,
    "build_appimage": BuildAppImage
}
