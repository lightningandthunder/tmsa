...
class Solunars(Frame):
    def __init__(self, base, filename):
        ...
        self.loc = Entry(self, base["location"], .3, .2, .3)
        ...
        self.options = Entry(self, "Default Return", .3, .4, .3)
        ...
        self.event = None
        Label(self, "Solar Returns", .15, .5, .3, anchor = tk.W)
        self.mainsolar = Checkbutton(self, "SSR", .3, .5, .1)
        self.demisolar = Checkbutton(self, "DSSR", .4, .5, .1)
        self.fqsolar = Checkbutton(self, "QSSR1", .5, .5, .1)
        self.lqsolar = Checkbutton(self, "QSSR3", .6, .5, .1)
        Label(self, "Lunar Returns", .15, .55, .3, anchor = tk.W)
        self.mainlunar = Checkbutton(self, "SLR", .3, .55, .1)
        self.demilunar = Checkbutton(self, "DSLR", .4, .55, .1)
        self.fqlunar = Checkbutton(self, "QSLR1", .5, .55, .1)
        self.lqlunar = Checkbutton(self, "QSLR3", .6, .55, .1)
        ...
        self.oneyear = Checkbutton(self, "All Selected Solunars For One Year", .3, .65, .4)
        ...
        Button(self, "Help", .5, .8, .2).bind("<Button-1>", lambda _ : delay(ShowHelp, HELP_PATH + r"\solunars.txt"))
        backbtn = Button(self, "Back", .7, .8, .20)
    ...
    def calculate(self):
        ...
        solunars = []   
        if self.mainsolar.checked: solunars.append("Solar Return")
        if self.demisolar.checked: solunars.append("Demi-Solar Return")
        if self.fqsolar.checked: solunars.append("First Quarti-Solar Return")
        if self.lqsolar.checked: solunars.append("Last Quarti-Solar Return")
        if self.mainlunar.checked: solunars.append("Lunar Return")
        if self.demilunar.checked: solunars.append("Demi-Lunar Return")
        if self.fqlunar.checked: solunars.append("First Quarti-Lunar Return")
        if self.lqlunar.checked: solunars.append("Last Quarti-Lunar Return")
        if not solunars:
        ...
        if self.oneyear.checked: self.burst(chart, solunars)  
        elif self.search.value == 0: self.asearch(chart,solunars)
        elif self.search.value == 1: self.fsearch(chart,solunars)
        elif self.search.value == 2: self.bsearch(chart, solunars)
    ...
    def asearch(self, chart, solunars):
        ...
        for sol in solunars:
            sl = sol.lower()
            if "solar" in sl: srs.append(sol)
            if "lunar" in sl: lrs.append(sol)
        cclass = "SR"
        if srs:
            cclass = "SR"
            date = calc_sun_crossing(sun, start - 184)
            if date > start:
                if date - start <= 15 and  srs[0] == "Solar Return": self.make_chart(chart, date, srs[0], cclass)
        ...
        if lrs:
            ...
            if date > start:
                if date - start <= 1.25 and  lrs[0] == "Lunar Return" : self.make_chart(chart, date, lrs[0], cclass)
    ...
    def make_chart(self, chart, date, chtype, cclass, show = True):
        ...
        cchart["day"] = d
        cchart["time"] = t
        cchart["name"] = f"{cchart['base_chart']['name']}" if chart.get("base_chart", None) else ""
        cchart["type"] = chtype
        cchart["class"] = cclass
        cchart["correction"] = 0
        cchart["zone"] = "UT"
