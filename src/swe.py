...
from ctypes import CDLL, byref, c_char_p, c_int, c_void_p, c_double, POINTER, c_char, create_string_buffer
from init import *
...
swe_revjul.argtypes = [c_double, c_int, POINTER(c_int), POINTER(c_int), POINTER(c_int), POINTER(c_double)]
swe_revjul.restype = c_void_p

swe_calc_ut = getattr(dll, "_swe_calc_ut@24")
swe_calc_ut.argtypes = [c_double, c_int, c_int, POINTER(c_double * 6), POINTER(c_char * 256)]

swe_get_ayanamsa_ex_ut = getattr(dll, "_swe_get_ayanamsa_ex_ut@20")
swe_get_ayanamsa_ex_ut.argtypes = [c_double, c_int, POINTER(c_double), POINTER(c_char * 256)] 

swe_houses_ex = getattr(dll, "_swe_houses_ex@40")
swe_houses_ex.argtypes = [c_double, c_int, c_double, c_double, c_int, POINTER(c_double * 13), POINTER(c_double * 10)]

swe_house_pos = getattr(dll, "_swe_house_pos@36")
swe_house_pos.argtypes = [c_double, c_double, c_double, c_int, POINTER(c_double * 2), POINTER(c_char * 256)]
swe_house_pos.restype = c_double

swe_azalt = getattr(dll, "_swe_azalt@40")
swe_azalt.argtypes = [c_double, c_int, POINTER(c_double * 3), c_double, c_double, POINTER(c_double * 3), POINTER(c_double * 3)]

swe_lat_to_lmt = getattr(dll, "_swe_lat_to_lmt@24")
swe_lat_to_lmt.argtypes = [c_double, c_double, POINTER(c_double), POINTER(c_char * 256)]
...
def calc_planet(ut, pl):
    ...
    for i in range(len(pos)):
        if i in [0, 1, 3]: result.append(pos[i])
    swe_calc_ut(ut, pl, 2048 + 256, byref(pos), byref(err))
    for i in range(len(pos)):
        if i in [0, 1]: result.append(pos[i])
    return result
...
def calc_cusps(ut, lat, long):
    cusps = (c_double * 13)()
    angles = (c_double * 10)()
    swe_houses_ex(ut, 64 * 1024, lat, long, ord("C"), byref(cusps), byref(angles))
    cc = []
    for c in cusps:
        cc.append(c)
    aa = []
    for i in range(len(angles)):
        if i in [2, 3, 4]: aa.append(angles[i])
    return [cc, aa]
