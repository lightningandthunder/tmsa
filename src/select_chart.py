...
class SelectChart(Frame): 
    def __init__(self):
        ...
        self.filename = ""
        self.first = Button(self, "Find Chart", .3, .05, .2)
        self.first.bind("<Button-1>", lambda _ : delay(self.find_file))
        self.first.focus()
        Button(self, "Show Chart", .5, .05, .2).bind("<Button-1>", lambda _ : delay(self.show_file))
        Button(self, "Export Chart", .3, .1, .2).bind("<Button-1>", lambda _ : delay(self.export_file))
        Button(self, "Recalculate Chart", .5, .1, .2).bind("<Button-1>", lambda _ : delay(self.recalculate)) 
        Button(self, "Delete Chart", .3, .15, .2).bind("<Button-1>", lambda _ : delay(self.delete_file))
        Button(self, "Solunars", .5, .15, .2).bind("<Button-1>", lambda _ : delay(self.solunars))
        Button(self, "Clear History", .3, .2, .2).bind("<Button-1>", lambda _ : delay(self.clear_history))
        Button(self, "Predictive Methods", .5, .2, .2).bind("<Button-1>", lambda _ : delay(NotImplemented))
        Button(self, "Help", .3, .25, .2).bind("<Button-1>", lambda _ : delay(ShowHelp, HELP_PATH + r"\selectchart.txt"))
        self.last = Button(self, "Back", .5, .25, .2)
        self.last.bind("<Button-1>", lambda _ : delay(self.destroy))
        self.last.bind("<Tab>", lambda _: delay(self.first.focus))
        self.status = Label(self, "", 0, .3, 1)
        self.reclbls = []
        self.recnames = []
        for i in range(11):
            self.reclbls.append(Label(self, "", 0, .35 + .05 * i, 1, font = ulfont))
            self.reclbls[i].bind("<Button-1>", lambda e : delay(self.set_file, e.widget.text))
            self.reclbls[i].bind("<Button-3>", lambda e : delay(self.unset_file, e.widget.text))
        Label(self, "Right click a chart to remove it from history.", 0, .9, 1)
        self.morebtn = Button(self, "More Charts", .4, .95, .2)
    ...
    def load_files(self):
        ...
        for i in range(len(self.recs)):
            ...
            if i < 11: self.reclbls[i].text = self.recnames[i]
    ...
    def more_files(self):
        if self.morebtn.disabled: return
    ...
    def sort_recent(self, save = True):
        i = self.recs.index(self.filename) if self.filename in self.recs else -1
        if i > -1: 
            self.recs.pop(i)
            self.recnames.pop(i)
        if save:
    ...
    def find_file(self):
        ...
        self.filename = ""
        self.fnlbl.text = ""
        name = tkfiledialog.askopenfilename(initialdir = CHART_PATH, filetypes = [("Chart Files", "*.dat")])
        if name:
            name = name.replace("/", "\\")
            self.filename = name
    ...
    def recalculate(self):
        ...
        try:
            ...
            self.status.error(f"Unable to open file '{os.path.basename(self.filename)}'.")
            
    def export_file(self):
        if self.filename == "": return  
        name = tkfiledialog.asksaveasfilename(initialfile = os.path.basename(self.filename), initialdir = CHART_PATH + "\\..\\..")
        if not name:
        ...
        try:
            ...
            self.status.error(f"Unable to export file: '{os.path.basename(self.filename)}'")
               
    def delete_file(self):
        if self.filename == "": return  
        if tkmessagebox.askyesno("Are you sure?", f"Delete chart '{self.fnlbl.text}'?"):
            try:
                ...
                self.status.error(f"Unable to delete file: '{os.path.basepath(filename)}'.")
    ...
    def solunars(self):
        ...
        try:
            with open(self.filename) as file:
                chart = json.load(file)
            self.sort_recent()
            if chart.get("base_chart", None): chart["basechart"] = None
            self.destroy()
    ...
    def migrate(self, filename):
        if filename.count("~") > 1: return filename
        ...
        try:
            ...
            self.status.error(f"Unable to convert '{os.path.basename(filename)}' to new format .")
