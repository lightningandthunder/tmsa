...
class ProgramOptions(Frame): 
    def __init__(self):
        ...
        Label(self, "Background", .2, .05, .15, anchor = tk.W )
        self.bgsample = Label(self, "", .35, .05, .05, anchor = tk.W)
        self.bgcolor = Entry(self, "", .4, .05, .2)
        self.bgcolor.focus()
        Button(self, "Choose", .6, .05, .1).bind("<Button-1>", lambda _ : delay(self.choose_color, "background"))
        Label(self, "Button", .2, .1, .15, anchor = tk.W)
        self.btnsample = Label(self, "", .35, .1, .05)
        self.btncolor = Entry(self, "", .4, .1, .2)
        Button(self, "Choose", .6, .1, .1).bind("<Button-1>", lambda _ : delay(self.choose_color, "button"))
        Label(self, "Text", .2, .15, .15, anchor = tk.W)
        self.txtsample = Label(self, "", .35, .15, .05)
        self.txtcolor = Entry(self, "", .4, .15, .2)
        Button(self, "Choose", .6, .15, .1).bind("<Button-1>", lambda _ : delay(self.choose_color, "text"))
        Label(self, "Error", .2, .2, .15, anchor = tk.W)
        self.errsample = Label(self, "", .35, .2, .05)
        self.errcolor = Entry(self, "", .4, .2, .2)
        Button(self, "Choose", .6, .2, .1).bind("<Button-1>", lambda _ : delay(self.choose_color, "error"))
        Label(self, "Data Entry Options", 0, .25, 1)
        Label(self, "Date Format", .2, .3, .2, anchor = tk.W)
        self.dtfmt= Radiogroup(self)
        Radiobutton(self, self.dtfmt, 0, "M D Y", .4, .3, .1)
        Radiobutton(self, self.dtfmt, 1, "D M Y", .5, .3, .15)
        Label(self, "Time Format", .2, .35, .2, anchor = tk.W)
        self.tmfmt= Radiogroup(self)
        Radiobutton(self, self.tmfmt, 0, "AM/PM", .4, .35, .1)
        Radiobutton(self, self.tmfmt, 1, "24 Hour", .5, .35, .1)
        Label(self, "Student Options", .2, .45, .2, anchor = tk.W)
        self.isstudent = Radiogroup(self)
        Radiobutton(self, self.isstudent, 1, "Yes", .4, .45, .1)
        Radiobutton(self, self.isstudent, 0, "No", .5, .45, .1)
        if os.path.exists(STUDENT_FILE): self.isstudent.value = 1
        Label(self,"Home Location", .15, .55, .15, anchor = tk.W)
        self.loc = Entry(self, "", .3, .55, .3)
        self.loc.bind("<KeyRelease>", lambda _: self.enable_find)
        Button(self,"Recent", .6, .55, .1).bind("<Button-1>", lambda _ : delay(self.recent_loc))
        self.findbtn = Button(self, "Find", .7, .55, .1)
        self.findbtn.bind("<Button-1>", lambda _ : delay(self.find))
        Label(self, "Lat D M S", .15, .6, .15, anchor = tk.W)
        self.latd = Entry(self, "", .3, .6, .1)
        self.latd.bind("<KeyRelease>", lambda _: delay(check_num, self.latd, 89))
        self.latm = Entry(self, "", .4, .6, .1)
        self.latm.bind("<KeyRelease>", lambda _: delay(check_num, self.latm, 59))
        self.lats = Entry(self, "0", .5, .6, .1)
        self.lats.bind("<KeyRelease>", lambda _: delay(check_num, self.lats, 59))
        self.latdir = Radiogroup(self)
        Radiobutton(self, self.latdir, 0, "North", .6, .6, .1)
        Radiobutton(self, self.latdir, 1, "South", .7, .6, .1)
        Label(self, "Long D M S", .15, .65, .15, anchor = tk.W) 
        self.longd = Entry(self, "", .3, .65, .1)
        self.longd.bind("<KeyRelease>", lambda _: delay(check_num, self.longd, 180))
        self.longm = Entry(self, "", .4, .65, .1)
        self.longm.bind("<KeyRelease>", lambda _: delay(check_num, self.longm, 59))
        self.longs = Entry(self, "0", .5, .65, .1)
        self.longs.bind("<KeyRelease>", lambda _: delay(check_num, self.longs, 59))
        self.longdir = Radiogroup(self)
        Radiobutton(self, self.longdir, 0, "East ", .6, .65, .1)
        Radiobutton(self, self.longdir, 1, "West ", .7, .65, .1)
        self.longdir.value = "1"
        Button(self, "Save", .2, .75, .15).bind("<Button-1>", lambda _ : delay(self.save))
        Button(self, "Restart", .35, .75, .15).bind("<Button-1>", lambda _ : delay(self.restart))
        Button(self, "Help", .5, .75, .15).bind("<Button-1>", lambda _ : delay(ShowHelp, HELP_PATH + r"\program_options.txt"))
        Button(self, "Back",.65, .75, .15).bind("<Button-1>", lambda _ : delay(self.destroy))
    ...
    def save(self):
        if not self.save_colors(): return False
        if not self.save_fmt(): return False
        if not self.save_student(): return False
        if not self.save_home(): return False
