...
class Locations(Frame):
    def __init__(self, listing, callback, offset=0):
        super().__init__()
        ...
        for i in range(17):
            self.location_labels.append(
                Label(self, '', 0, i * 0.05 + 0.05, 1, font=ulfont)
            )
            self.location_labels[i].bind(
        ...
        for i in range(min(17, len(self.listing) - self.offset)):
            self.location_labels[i].text = self.names[i + self.offset]
        Label(
            self,
        ...
        Button(self, 'Not Found', 0, 0.95, 0.2).bind(
    ...
    def next_page(self):
        Locations(self.listing, self.callback, self.offset + 17)
    ...
    def remove(self, value):
        ...
        for i in range(17):
            self.location_labels[i].text = ''
        for i in range(min(17, len(self.listing))):
            self.location_labels[i].text = self.names[i]
        try:
