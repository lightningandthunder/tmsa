...
class Ingresses(Frame): 
    def __init__(self):
        ...
        Radiobutton(self, self.search, 0, "Active Charts", .3, .05, .2).bind("<Button-1>", lambda _: delay(self.toggle_year, 0))
        Radiobutton(self, self.search, 1, "Forwards", .5, .05, .15).bind("<Button-1>", lambda _: delay(self.toggle_year, 1))
        Radiobutton(self, self.search, 2, "Backwards", .65, .05, .15).bind("<Button-1>", lambda _: delay(self.toggle_year, 2))
        self.search.value = 0
        self.search.focus()
        self.init = False
        Label(self, "Date " + DATE_FMT, .15, .1, .15, anchor = tk.W)
        if DATE_FMT == "D M Y":
            self.dated = Entry(self, now.strftime("%d"), .3, .1, .1)
            self.datem = Entry(self, now.strftime("%m"), .4, .1, .1)
        else:
            self.datem = Entry(self, now.strftime("%m"), .3, .1, .1)
            self.dated = Entry(self, now.strftime("%d"), .4, .1, .1)
        self.datey = Entry(self, now.strftime("%Y"), .5, .1, .1)
        ...
        self.datey.bind("<KeyRelease>", lambda _: delay(check_snum, self.datem, -2999, 3000))
        self.bce = Checkbutton(self, "BC/BCE", .6, .1, .1, focus = False)
        self.old = Checkbutton(self, "OS", .7, .1, .1, focus = False)
        Label(self, "Time UT H M S ", .15, .15, .15, anchor = tk.W)
        h = "%I" if TIME_FMT == "AM/PM" else "%H"
        self.timeh = Entry(self, now.strftime(h), .3, .15, .1)
        maxh = 12 if TIME_FMT == "AM/PM" else 23
        self.timeh.bind("<KeyRelease>", lambda _: delay(check_num, self.timeh, maxh))
        self.timem = Entry(self, now.strftime("%M"), .4, .15, .1)
        self.timem.bind("<KeyRelease>", lambda _: delay(check_num, self.timem, 59))
        self.times = Entry(self,  now.strftime("%S"), .5, .15, .1)
        self.times.bind("<KeyRelease>", lambda _: delay(check_num, self.times, 59))
        self.tmfmt = Radiogroup(self)
        if TIME_FMT == "24 Hour":
            Radiobutton(self, self.tmfmt, 2, "24 Hour Clock", .6, .15, .2)
            self.tmfmt.value = 2
        else:
            Radiobutton(self, self.tmfmt, 0, "AM", .6, .15, .1)
            Radiobutton(self, self.tmfmt, 1, "PM", .7, .15, .1)
            if now.strftime("%p") == "PM":
                    self.tmfmt.value = 1
        Label(self, "Location", .15, .2, .15, anchor = tk.W)
        self.loc = Entry(self, "", .3, .2, .3)
        self.loc.bind("<KeyRelease>", lambda _: delay(self.enable_find))
        Button(self,"Recent", .6, .2, .1).bind("<Button-1>", lambda _ : delay(self.recent_loc))
        self.findbtn = Button(self, "Find", .7, .2, .1)
        self.findbtn.bind("<Button-1>", lambda _ : delay(self.find))
        Label(self, "Lat D M S", .15, .25, .15, anchor = tk.W)
        self.latd = Entry(self, "", .3, .25, .1)
        self.latd.bind("<KeyRelease>", lambda _: delay(check_num, self.latd, 89))
        self.latm = Entry(self, "", .4, .25, .1)
        self.latm.bind("<KeyRelease>", lambda _: delay(check_num, self.latm, 59))
        self.lats = Entry(self, "0", .5, .25, .1)
        self.lats.bind("<KeyRelease>", lambda _: delay(check_num, self.lats, 59))
        self.latdir = Radiogroup(self)
        Radiobutton(self, self.latdir, 0, "North", .6, .25, .1)
        Radiobutton(self, self.latdir, 1, "South", .7, .25, .1)
        Label(self, "Long D M S", .15, .3, .15, anchor = tk.W) 
        self.longd = Entry(self, "", .3, .3, .1)
        self.longd.bind("<KeyRelease>", lambda _: delay(check_num, self.longd, 180))
        self.longm = Entry(self, "", .4, .3, .1)
        self.longm.bind("<KeyRelease>", lambda _: delay(check_num, self.longm, 59))
        self.longs = Entry(self, "0", .5, .3, .1)
        self.longs.bind("<KeyRelease>", lambda _: delay(check_num, self.longs, 59))
        self.longdir = Radiogroup(self)
        Radiobutton(self, self.longdir, 0, "East ", .6, .3, .1)
        Radiobutton(self, self.longdir, 1, "West ", .7, .3, .1)
        self.longdir.value = 1
        Label(self, "Notes", .15, .35, .15, anchor = tk.W)
        self.notes = Entry(self, "", .3, .35, .3)
        Label(self, "Options", .15, .4, .15, anchor = tk.W)
        self.options = Entry(self, "Default Ingress", .3, .4, .3)
        Button(self, "Select", .6, .4, .1).bind("<Button-1>", lambda _ : delay(self.select_options))
        Button(self, "Temporary", .7, .4, .1).bind("<Button-1>", lambda _ : delay(self.temp_options))
        Label(self, "Optional Event", .15, .45, .15)
        Button(self, "New Chart", .3, .45, .15).bind("<Button-1>", lambda _ : delay(self.make_event))
        Button(self, "Find Chart", .45, .45, .15).bind("<Button-1>", lambda _ : delay(self.more_files))
        self.event = None
        Label(self, "Solar Ingress", .15, .5, .3, anchor = tk.W)
        self.capsolar = Checkbutton(self, "Capricorn", .3, .5, .13)
        self.cansolar = Checkbutton(self, "Cancer", .43, .5, .09)
        self.arisolar = Checkbutton(self, "Aries", .52, .5, .09)
        self.libsolar = Checkbutton(self, "Libra", .61, .5, .09)
        Label(self, "Lunar Ingress", .15, .55, .3, anchor = tk.W)
        self.caplunar = Checkbutton(self, "Capricorn", .3, .55, .13)
        self.canlunar = Checkbutton(self, "Cancer", .43, .55, .09)
        self.arilunar = Checkbutton(self, "Aries", .52, .55, .09)
        self.liblunar = Checkbutton(self, "Libra", .61, .55, .09)
        Button(self, "Select All", .3, .6, .2).bind("<Button-1>", lambda _ : delay(self.all_charts, True))
        Button(self, "Clear Selections", .5, .6, .2).bind("<Button-1>", lambda _ : delay(self.all_charts, False))
        self.oneyear = Checkbutton(self, "All Selected Ingresses For One Year", .3, .65, .4)
        self.oneyear.config(state=tk.DISABLED)
        self.istemp = Radiogroup(self)
        Radiobutton(self, self.istemp, 0, "Permanent Charts", .3, .7, .25)
        Radiobutton(self, self.istemp, 1, "Temporary Charts", .5, .7, .25)
        self.istemp.value = 1
        Button(self, "Calculate", .1, .8, .2).bind("<Button-1>", lambda _ : delay(self.calculate))
        Button(self, "Clear", .3, .8, .2).bind("<Button-1>", lambda _ : delay(self.clear))
        Button(self, "Help", .5, .8, .2).bind("<Button-1>", lambda _ : delay(ShowHelp, HELP_PATH + r"\ingresses.txt"))
        Button(self, "Back", .7, .8, .20).bind("<Button-1>", lambda _ : delay(self.destroy))
        self.status = Label(self, "", 0, .9, 1)
        if HOME_LOC:
    ...
    def more_finish(self, selected):
        if selected: 
            ...
            filename = tkfiledialog.askopenfilename(initialdir = CHART_PATH, filetypes = [("Chart Files", "*.dat")])              
        ...
        try:
            ...
            return self.status.error(f"Unable to open file '{os.path.basename(self.event)}'.")
        y = echart["year"]
        m = echart["month"]
        d = echart["day"] 
        ...
        if TIME_FMT == "AM/PM":
            ...
            if hr > 12: self.timeh.text = hr - 12  
            if hr == 0: self.timeh.text = 12
    ...
    def make_event(self): 
        ...
        self.status.text = f"Event chart will be calculated using options '{self.eopt}'"
    ...
    def recent_loc(self):
        try:
            with open(LOCATIONS_FILE, "r") as datafile:
                recs = json.load(datafile)
            if len(recs) == 0 : return self.status.error("No saved locations.")
    ...
    def find(self): 
        ...
        try:
            ...
            return self.status.error(f"Unable to connect to location database.", self.loc)
        if not location:
            return self.status.error(f"'{self.loc.text}' not in database.", self.loc)
    ...
    def calculate(self): 
        ...
        try:
            lat = int(self.latd.text) + int(self.latm.text)/60 + int(self.lats.text or "0")/3600
        except Exception:
            return self.status.error("Latitude must be numeric.", self.latd)
        if lat<0 or lat > 89.99:
            return self.status.error(f"Latitude must be between 0{DS} and 89{DS}59'59{DQ}.", self.latd)
        if self.latdir.value == 1: lat = -lat
        chart["latitude"] = lat
        try:
            long = int(self.longd.text) + int(self.longm.text)/60 + int(self.longs.text or "0")/3600
        except Exception:
            return self.status.error("Longitude must be numeric.", self.longd)
        if long < 0 or long > 180:
            return self.status.error(f"Longitude must be between 0{DS} and 180{DS}.", self.longd)
        ...
        if not self.bce.checked and y < 1:
            ...
            self.bce.checked = True    
        if y < 1 or y > 3000:
            return self.status.error("Year must be between 1 and 3000.", self.datey)
        if self.bce.checked: y = -y + 1
        chart["year"] = y
        if  m < 1 or m > 12:
            return self.status.error("Month must be between 1 and 12.", self.datem)
        chart["month"] = m
        if d < 1 or d > 31:
            return self.status.error("Day must be between 1 and 31.", self.dated)
            return
        chart["day"] = d
        ...
        ingresses = []
        if self.capsolar.checked: ingresses.append("Capsolar")
        if self.cansolar.checked: ingresses.append("Cansolar")
        if self.arisolar.checked: ingresses.append("Arisolar")
        if self.libsolar.checked: ingresses.append("Libsolar")
        if self.caplunar.checked: ingresses.append("Caplunar")
        if self.canlunar.checked: ingresses.append("Canlunar")
        if self.arilunar.checked: ingresses.append("Arilunar")
        if self.liblunar.checked: ingresses.append("Liblunar")
        if not ingresses:
        ...
        if self.event:
            if self.event == "<":
                ...
                date = julday(cchart["year"], cchart["month"], cchart["day"], cchart["time"], cchart["style"])
        ...
        if self.oneyear.checked: self.burst(chart, ingresses)  
        elif self.search.value == 0: self.asearch(chart,ingresses)
        elif self.search.value == 1: self.fsearch(chart, ingresses)
        elif self.search.value == 2: self.bsearch(chart, ingresses)
        else: self.status.error("No search direction selected.")
            
        
    def fsearch(self, chart, ingresses):
        start = julday(chart["year"], chart["month"], chart["day"], chart["time"], chart["style"])
        for ing in ingresses:
            if "Ari" in ing: target = 0
            if "Can" in ing: target = 90
            if "Lib" in ing: target = 180
            if "Cap" in ing: target = 270
            if "solar" in ing: date = calc_sun_crossing(target, start)
            if "lunar" in ing: date = calc_moon_crossing(target, start)
    ...
    def burst(self, chart, ingresses): 
        start = julday(chart["year"], chart["month"], chart["day"], chart["time"], chart["style"])
        if self.search.value == 2: start -= 366
        for ing in ingresses:
            if "solar" in ing:
                if "Ari" in ing: target = 0
                if "Can" in ing: target = 90
    ...
    def asearch(self, chart, ingresses):
        ...
        for ing in ingresses:
            ...
            if "lunar" in ing:
                ...
                found = True
                self.make_chart(chart, date, ing)
        if not found: self.status.error("No charts found.")
