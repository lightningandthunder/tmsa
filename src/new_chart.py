...
class NewChart(Frame):
    def __init__(self, oldchart=None):
        ...
        self.datey.bind(
            '<KeyRelease>',
            lambda _: delay(check_snum, self.datey, -2999, 3000),
        )
        ...
        self.tzch.bind(
            '<KeyRelease>', lambda _: delay(check_num, self.tzch, 23)
        )
        self.tzcm = Entry(self, '0', 0.4, 0.45, 0.1)
        self.tzcm.bind(
            '<KeyRelease>', lambda _: delay(check_num, self.tzcm, 59)
        )
        self.tzcs = Entry(self, '0', 0.5, 0.45, 0.1)
        self.tzcs.bind(
            '<KeyRelease>', lambda _: delay(check_num, self.tzcs, 59)
        )
