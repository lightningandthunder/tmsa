...
class Locations(Frame):
    def __init__(self, listing, callback):
        ...
        self.names = [f"{item[0]} {fmt_lat(item[1])} {fmt_long(item[2])}" for item in listing]
        self.locdict = {}
        for i in range(len(self.names)): self.locdict[self.names[i]] = self.listing[i]
        self.lbls = []
        Label(self, "Select Location", 0, 0, 1)
        for i in range(17): 
            self.lbls.append(Label(self, "", 0, i * .05 + .05, 1, font = ulfont))
            self.lbls[i].bind("<Button-1>", lambda e : delay(self.set_result, e.widget.text))
            self.lbls[i].bind("<Button-3>", lambda e : delay(self.remove, e.widget.text))   
        for i in range(min(17, len(self.listing))):
            self.lbls[i].text = self.names[i] 
        Label(self, "Right click a location to remove it from history.", 0, .9, 1)
        Button(self, "Not Found", .4, .95, .2).bind("<Button-1>", lambda _: delay(self.not_found))
    ...
    def remove(self, value):
        ...
        for i in range(17): self.lbls[i].text = ""
        for i in range(min(17, len(self.listing))):
            self.lbls[i].text = self.names[i]
        try:
